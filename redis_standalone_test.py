...
def get_redis_standalone_ids():
    yml_info = yamlUtil("./cluster.yaml").read_yaml()
    print(yml_info)
    return yml_info['redis_standalone']['id']
...
def teardown_function():
    print("测试redis_standalone连通性")
    time.sleep(60)
    clusters = get_redis_standalone_ids()
...
def describle_cluster_master_slave_nodes(cluster):
    nodes = describle_cluster_nodes(cluster)
    return [node_id for node_id in list(nodes.keys()) if nodes[node_id] == '']
...
@pytest.mark.parametrize('cluster', get_redis_standalone_ids())
@allure.title('测试停止RedisStandalone集群')  # 设置用例标题
...
@pytest.mark.parametrize('cluster', get_redis_standalone_ids())
@allure.title('测试启动RedisStandalone集群')  # 设置用例标题
@allure.severity(allure.severity_level.CRITICAL)  # 设置用例优先级
...
@allure.title('测试增加RedisStandalone集群2个副本节点')  # 设置用例标题
@allure.severity(allure.severity_level.CRITICAL)  # 设置用例优先级
@pytest.mark.redis5
@pytest.mark.redis4
def test_add_cluster_slave_nodes(cluster):
    params = get_common_params()
...
@allure.title('测试增加RedisStandalone集群剩余副本节点')  # 设置用例标题
@allure.severity(allure.severity_level.CRITICAL)  # 设置用例优先级
@pytest.mark.redis5
@pytest.mark.redis4
def test_add_cluster_slave_odd_nodes(cluster):
    params = get_common_params()
    params['action'] = "AddClusterNodes"
    params['cluster'] = cluster
    params['node_count'] = 6
...
@allure.title('删除RedisStandalone的副本节点')  # 设置用例标题
@allure.severity(allure.severity_level.CRITICAL)  # 设置用例优先级
@pytest.mark.redis5
@pytest.mark.redis4
@pytest.mark.run(order=5)
def test_delete_cluster_slave_nodes(cluster):
    nodes = describle_cluster_master_slave_nodes(cluster)
    print(nodes)
    params = get_common_params()
    params['action'] = "DeleteClusterNodes"
    params['cluster'] = cluster
    # params['nodes'] = json.dumps(nodes[0:8])

    # node1,node2 = random.choice(nodes.items())
    # params['nodes.1'] = nodes[0]
...
@allure.title('resize RedisCLuster的节点')  # 设置用例标题
@allure.severity(allure.severity_level.CRITICAL)  # 设置用例优先级
...
if __name__ == '__main__':
    pytest.main(['-vs', './redis_standalone_test.py::test_resize_redis_cluster_master_nodes'])
    # teardown_function()
    # test_change_vxnet_redis_cluster
    # describle_cluster('cl-ubyfblow')
